"""Paired-map evaluation against a canonical uniformly random policy."""
...
def evaluate_paired_vs_random(
    ...
    *,
    ...
    def step(carry, _):
        states, rng, memory, finished, outcomes = carry
        ...
        random_observation = jax.tree.map(
            lambda zero, one: jnp.where(
        ...
        rng = split_keys[0]
        random_actions = jax.vmap(_random_action)(
            split_keys[1:], random_observation, states.board_mask
        )
        actions_zero = jnp.where(network_is_zero[:, None], network_actions, random_actions)
        actions_one = jnp.where(network_is_zero[:, None], random_actions, network_actions)
        ...
        return (states, rng, memory, finished, outcomes), None

    (_, key, _, finished, outcomes), _ = jax.lax.scan(
        step, (states, key, memory, finished, outcomes), None, length=truncation
    )
