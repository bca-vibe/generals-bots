...
def generate_grid(
    ...
    if pad_h > 0 or pad_w > 0:
        grid = jnp.pad(
            ...
            ((0, pad_h), (0, pad_w)),
            mode='constant',
            # The graded runner never pads. Training pools do, and the bot
            # knows the real H/W from the competition handshake, so retain a
            # distinct impassable sentinel for its board mask.
            constant_values=-3,
        )
