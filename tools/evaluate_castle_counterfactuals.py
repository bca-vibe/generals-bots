...
def _select_by_seat(zero, one, seats):
    return jax.tree.map(
        lambda z, o: jnp.where(
            seats.reshape(seats.shape + (1,) * (z.ndim - seats.ndim)), z, o
        ),
        one,
        zero,
    )
...
def collect_opportunity_batch(network, pool, key, config, environment):
    ...
    def step(carry, _):
        ...
        replace = opportunity_by_bin & (
            reservoir_draw < 1.0 / jnp.maximum(next_seen, 1)
        )
...
def _expand_pair_axis(value, repetitions: int):
    ...
    return jnp.broadcast_to(
        value[:, None, None], (states, repetitions, 2, *value.shape[1:])
    ).reshape(states * repetitions * 2, *value.shape[1:])
...
def evaluate_pair_batch(
    ...
    *,
    ...
    observations_zero = jax.vmap(lambda state: get_observation(state, 0))(
        source_states
    )
    observations_one = jax.vmap(lambda state: get_observation(state, 1))(
        source_states
    )
    observations = jax.tree.map(
        lambda zero, one: jnp.concatenate([zero, one]),
    ...
    board_masks = jnp.concatenate(
        [source_states.board_mask, source_states.board_mask]
    )
    ...
    opponent_logits = jnp.where(
        actor_seats[:, None] == 0, logits_one, logits_zero
    )
    ...
    best_build_probability = jnp.take_along_axis(
        probabilities, build_indices[:, None], axis=-1
    )[:, 0]
    control_probability = jnp.take_along_axis(
        probabilities, control_indices[:, None], axis=-1
    )[:, 0]
    best_build_logit = jnp.take_along_axis(
        actor_logits, build_indices[:, None], axis=-1
    )[:, 0]
    best_control_logit = jnp.take_along_axis(
        actor_logits, control_indices[:, None], axis=-1
    )[:, 0]
    best_build_rank = 1 + jnp.sum(
        actor_logits > best_build_logit[:, None], axis=-1
    )
    legal_build_sites = jnp.sum(
        actor_masks[:, BUILD_START:PASS_INDEX], axis=-1
    )
    ...
    actor_observations = _select_by_seat(
        observations_zero, observations_one, actor_seats
    )
    site_cost_grids = jax.vmap(build_cost_grid_from_observation)(
        actor_observations
    )
    ...
    own_structures = (
        actor_observations.generals | actor_observations.castles
    ) & actor_observations.owned_cells
    own_generals = actor_observations.generals & actor_observations.owned_cells
    distance_to_general = jax.vmap(_minimum_manhattan)(
        own_generals, site_rows, site_cols
    )
    distance_to_structure = jax.vmap(_minimum_manhattan)(
        own_structures, site_rows, site_cols
    )
    ...
    distance_to_enemy_land = jax.vmap(_minimum_manhattan)(
        true_enemy, site_rows, site_cols
    )
    own_castles = jnp.sum(
        actor_observations.castles & actor_observations.owned_cells, axis=(-2, -1)
    )
    ...
    flat_states = jax.tree.map(
        lambda value: _expand_pair_axis(value, repetitions), source_states
    )
    ...
    branch = jnp.broadcast_to(
        jnp.arange(2)[None, None, :], (count, repetitions, 2)
    ).reshape(-1)
    actor_action_index = jnp.where(
        branch == BRANCH_BUILD, build_flat, control_flat
    )
    ...
    intervention_timestep, _ = jax.vmap(
        lambda state, action: environment.step(state, action, pool)
    )(flat_states, actions)
    ...
    post_observations_zero = jax.vmap(lambda state: get_observation(state, 0))(
        states
    )
    post_observations_one = jax.vmap(lambda state: get_observation(state, 1))(
        states
    )
    ...
    post_masks = jax.vmap(legal_action_mask)(
        post_observations, post_board_masks
    )
    post_histories = temporal_input(post_memories)
    _, post_values, _ = jax.vmap(network.forward)(
        post_augmented, post_histories, post_masks
    )
    games = states.armies.shape[0]
    post_actor_values = jnp.where(
        actor_flat == 0, post_values[:games], post_values[games:]
    )
    ...
    survived_horizons = jnp.zeros(
        (states.armies.shape[0], len(HORIZONS)), dtype=jnp.bool_
    )
    ...
    def step(carry, scan_index):
        ...
        action_zero_index = jax.vmap(jax.random.categorical)(
            keys_zero, logits[:games]
        )
        action_one_index = jax.vmap(jax.random.categorical)(
            keys_one, logits[games:]
        )
        actions = jnp.stack(
            [
        ...
        new_outcome = jnp.where(
            actor_won, 1.0, jnp.where(actor_lost, 0.0, 0.5)
        )
        outcome = jnp.where(newly_done, new_outcome, outcome)
        relative_turn = scan_index + 2
        finish_relative_turn = jnp.where(
            newly_done, relative_turn, finish_relative_turn
        )
        done |= newly_done

        owns_site = states.ownership[
            flat_index, actor_flat, rows_flat, cols_flat
        ]
        site_is_castle = states.castles[flat_index, rows_flat, cols_flat]
        ever_lost_site |= (branch == BRANCH_BUILD) & ~owns_site
        produced = (
        ...
        newly_paid_back = (
            (payback_relative_turn < 0) & (production_ticks >= cost_flat)
        )
        payback_relative_turn = jnp.where(
            newly_paid_back, relative_turn, payback_relative_turn
        )
    ...
    final_owns_site = final_states.ownership[
        flat_index, actor_flat, rows_flat, cols_flat
    ]
    results = {
        ...
        "survived_horizons": final[10].reshape(
            count, repetitions, 2, len(HORIZONS)
        ),
...
def _choose_stratified(found, requested, seed):
    ...
    bin_ids = np.broadcast_to(
        np.arange(TURN_BINS)[None, None, :], found.shape
    ).reshape(-1)
    ...
    if len(chosen) < requested:
        raise RuntimeError(
            f"Only {len(chosen)} sampled legal opportunities were available; "
...
def _fixed_bins(values, edges, labels):
    return np.asarray(labels, dtype=object)[
        np.clip(np.digitize(values, edges, right=True), 0, len(labels) - 1)
    ]
...
def summarize(features, results, metadata, seed):
    ...
    heldout_first_to_second = np.where(
        select_first[:, None], build[:, half:], control[:, half:]
    ).mean() - control[:, half:].mean()
    heldout_second_to_first = np.where(
        select_second[:, None], build[:, :half], control[:, :half]
    ).mean() - control[:, :half].mean()
    ...
    for index in detail_indices:
        row = {name: np.asarray(value)[index].item() for name, value in features.items()}
        row.update(
            {
    ...
    return {
        ...
        "state_level_effects": {
            ...
            "split_half_positive_sign_agreement": float(
                np.mean((first_delta > 0) == (second_delta > 0))
            ),
            "heldout_oracle": {
                "first_half_select_rate": float(select_first.mean()),
                "second_half_uplift_when_selected_by_first": float(
                    heldout_first_to_second
                ),
                "second_half_select_rate": float(select_second.mean()),
                "first_half_uplift_when_selected_by_second": float(
                    heldout_second_to_first
                ),
                "symmetric_uplift": float(
                    (heldout_first_to_second + heldout_second_to_first) / 2
                ),
            },
        },
        "castle_mechanics": {
            "uninterrupted_ownership_survival_rate": {
                str(horizon): float(survival[:, :, index].mean())
                for index, horizon in enumerate(HORIZONS)
            },
            "owns_site_at_game_end_rate": float(
                results["owns_site_at_end"][:, :, BRANCH_BUILD].mean()
            ),
            "gross_production_payback_rate": float(np.mean(payback >= 0)),
            "payback_relative_turn_median": (
                float(np.median(payback[payback >= 0]))
                if np.any(payback >= 0)
                else None
            ),
            "gross_castle_production_ticks_mean": float(
                results["production_ticks"][:, :, BRANCH_BUILD].mean()
            ),
        },
        "policy_alignment": {
            "aggregate_build_probability_geometric_mean": float(
                10 ** np.mean(log_build_probability)
            ),
            "aggregate_build_probability_median": float(
                np.median(build_probability)
            ),
            "best_build_rank_median": float(
                np.median(features["best_build_rank"])
            ),
            "best_build_logit_margin_median": float(
                np.median(features["best_build_logit_margin"])
            ),
            "spearman_log_build_probability_vs_paired_delta": _spearman(
                log_build_probability, deltas
            ),
            "spearman_build_logit_margin_vs_paired_delta": _spearman(
                features["best_build_logit_margin"], deltas
            ),
            "mean_probability_positive_estimate_states": (
                float(build_probability[deltas > 0].mean())
                if np.any(deltas > 0)
                else None
            ),
...
def main():
    ...
    if args.expected_iteration is not None and iteration != args.expected_iteration:
        raise ValueError(
            f"Expected checkpoint iteration {args.expected_iteration}, loaded {iteration}"
        )
    if stage_index != len(config.curriculum) - 1:
        raise ValueError(
            f"Checkpoint stage {stage_index} is not final stage "
    ...
    if source_checkpoint == args.checkpoint and source_config_path == args.config:
        ...
        source_raw, source_ema, source_iteration, source_stage_index = load_checkpoint(
            source_config, source_checkpoint
        )
    if source_stage_index != len(source_config.curriculum) - 1:
        raise ValueError(
            f"Source checkpoint stage {source_stage_index} is not final stage "
            f"{len(source_config.curriculum) - 1}"
        )
        ...
        candidate_batches.append(
            {
                ...
                "opportunity_steps": np.asarray(
                    collected["opportunity_steps"]
                ).reshape(-1),
            }
        )
        print(
    ...
    candidates = {
        ...
        "memory_zero": _tree_concat(
            [batch["memory_zero"] for batch in candidate_batches]
        ),
        "memory_one": _tree_concat(
            [batch["memory_one"] for batch in candidate_batches]
        ),
        "found": np.concatenate([batch["found"] for batch in candidate_batches]),
        "opportunity_steps": np.concatenate(
            [batch["opportunity_steps"] for batch in candidate_batches]
        ),
    }
    candidate_shape = (
    ...
    selected_indices, selected_bins = _choose_stratified(
        found_shaped, args.opportunities, args.seed + 17
    )
    ...
    selected_actor_seats = (
        (selected_indices // TURN_BINS) % 2
    ).astype(np.int32)
    selected_source_slots = selected_indices % (
        args.collection_batch_size * 2 * TURN_BINS
    )
    selected_source_batches = selected_indices // (
        args.collection_batch_size * 2 * TURN_BINS
    )
    selected_metadata = {
    ...
    print(
        ...
        + json.dumps(
            {
                f"{index * 200}-{index * 200 + 199}": int(
                    np.sum(selected_bins == index)
                )
    ...
    for turn_bin in range(TURN_BINS):
        ...
        for start in range(0, len(bin_selection), args.pair_batch_size):
            ...
            batch_key = jax.random.fold_in(
                jax.random.PRNGKey(args.seed + 1), completed
            )
            ...
            result_batches.append(
                {name: np.asarray(jax.device_get(value))[:valid] for name, value in results.items()}
            )
            metadata_batches.append(
                {name: value[indices][:valid] for name, value in selected_metadata.items()}
            )
            completed += valid
            print(
    ...
    features = {
        name: np.concatenate([batch[name] for batch in feature_batches])
        for name in feature_batches[0]
    }
    results = {
        name: np.concatenate([batch[name] for batch in result_batches])
        for name in result_batches[0]
    }
    metadata = {
        name: np.concatenate([batch[name] for batch in metadata_batches])
        for name in metadata_batches[0]
    }
    ...
    report = {
        ...
        "source_sampling": {
            ...
            "legal_opportunity_steps_seen": int(
                candidates["opportunity_steps"].sum()
            ),
            "selected_by_turn_bin": {
                f"{index * 200}-{index * 200 + 199}": int(
                    np.sum(metadata["turn_bin"] == index)
                )
                for index in range(TURN_BINS)
            },
        },
