...
def test_generals_distance():
    ...
    for k in keys:
        ...
        # Mountains and the training-only -3 padding sentinel are impassable.
        # On the graded runner there is no padding; its exact rectangle has only
        # the competition engine's ordinary -2 mountain representation.
        walking.append(int(bfs_distance_field(grid >= 0, a)[b]))
