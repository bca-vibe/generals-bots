"""CPU inference for the iteration-1260 EMA policy from smoke_8xh100.

This intentionally has no Equinox dependency: the competition image includes
JAX but not the training framework. The implementation mirrors the original
legacy_38 observation memory and seven-layer transformer exactly.
"""
...
SCALED_CHANNELS = np.array(
    [0, 1, 2, 3, 14, 16, 17, 18, 19, 20, *range(24, 38)], dtype=np.int32
)
...
def _layer_norm(x, weight, bias):
    ...
    return (weight.astype(jnp.float32) * normalized + bias.astype(jnp.float32)).astype(
        original_dtype
    )
...
def _policy_action(parameters, observation, temporal_history, legal_mask):
    ...
    patches = (
        observation.reshape(38, 7, 3, 7, 3)
        .transpose(1, 3, 0, 2, 4)
        .reshape(49, 342)
    )
    patch_tokens = _linear(
        patches, parameters["patch_embedding.weight"], parameters["patch_embedding.bias"]
    )
    ...
    for index in range(7):
        ...
        query, key, value = (
            item.transpose(1, 0, 2) for item in (query, key, value)
        )
        scores = query @ key.transpose(0, 2, 1) / math.sqrt(56)
        attention = jax.nn.softmax(scores.astype(jnp.float32), axis=-1).astype(
            jnp.bfloat16
        )
    ...
    tokens = _layer_norm(
        tokens, parameters["output_norm.weight"], parameters["output_norm.bias"]
    )
    ...
    spatial_logits = (
        patch_logits.reshape(7, 7, 9, 3, 3)
        .transpose(2, 0, 3, 1, 4)
        .reshape(-1)
    )
    pass_logit = _linear(
        tokens[0], parameters["pass_head.weight"], parameters["pass_head.bias"]
    ).astype(jnp.float32)
    logits = jnp.concatenate((spatial_logits, pass_logit))
    return jnp.argmax(jnp.where(legal_mask, logits, -1e9)).astype(jnp.int32)


class Smoke1260Agent:
    ...
    def warmup(self) -> None:
        observation = jnp.zeros((38, BOARD_SIZE, BOARD_SIZE), dtype=jnp.float32)
    ...
    def _augment(self, observation):
        ...
        self.known_mountains |= mountains | padding
        # This is the legacy_38 checkpoint: it did not infer newly built castles
        # from a structure-in-fog cell that was previously known plain.
        self.known_castles |= castles

        self.last_seen_enemy_army = np.where(
            opponent, enemy_army, self.last_seen_enemy_army
        )
        self.last_seen_enemy_age = np.where(
            opponent, 0.0, self.last_seen_enemy_age + 1.0
        )
        ...
        augmented = np.concatenate(
            (channels, self.own_army_deltas, self.enemy_army_deltas), axis=0
        )
        temporal = np.stack(
            (self.opponent_army_history, self.opponent_land_history), axis=0
        )

        self.previous_own_army = own_army
        self.previous_enemy_army = enemy_army
        return augmented, temporal, armies, owned, opponent, mountains, castles, generals

    def _legal_mask(self, armies, owned, opponent, mountains, castles, generals):
